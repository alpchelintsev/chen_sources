...
// * The program for constructing the arc of the trajectory of         *
// * the Chen system in a given time interval, image in GIF format     *
// * (with comments in Russian). The program generates the text file   *
// * image_gnuplot.txt with commands for Gnuplot.                      *
// *********************************************************************
// * Программа построения дуги траектории динамической системы Чена на *
// * заданном отрезке времени (картинка в формате GIF).                *
// * Программа генерирует текстовый файл image_gnuplot.txt с командами *
// * для Gnuplot.                                                      *
// *********************************************************************
...
void calc(mpreal &x, mpreal &y, mpreal &z, const mpreal &T, int way = 1)
{
    ...
    f << "set term gif size 8000, 8000 font \"Times-Roman,90\"\nset output \"chen_traj_img.gif\"\n";
    f << "set xlabel \"x\"\nset ylabel \"y\"\nset zlabel \"z\"\n";
    f << "set xrange [-40:40]\nset yrange [-40:40]\nset zrange [0:60]\n";
    f << "splot '-' with line\n";
    f << x.toString(10) << " " << y.toString(10) << " " << z.toString(10) << endl;
    bool fl_rp;
    do
    {
        if(FL_CALC)
            delta_t = get_delta_t(x, y, z)/10;
        else
            delta_t = step_t;
        t += delta_t;
        if(t < T)
